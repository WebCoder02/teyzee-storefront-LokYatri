document.addEventListener("DOMContentLoaded", function () {
    // Fetch store.json and split into veg/not-veg arrays
    fetch("store.json")
        ...
        .then(data => {
            ...
            if (vegData.length > 0) populateMenu(vegData, "veg");
            if (nonvegData.length > 0) populateMenu(nonvegData, "nonveg");
        })
    ...
    data.forEach(dish => {
        ...
        dishElement.innerHTML = `
            <i class="fa fa-circle" style="font-size:14px; color: ${dish.veg ? 'green' : 'red'};">
                <span class="${dish.veg ? 'veg' : 'non-veg'}">
                    ${dish.veg ? 'VEG' : 'NOT-VEG'}
                </span>
            </i>
